...
namespace WordStats
{
    public sealed class WordStatsDictionaryImpl : IWordStats
    {
        ...
        public IEnumerable<string> GetLargestFiveWords()
        {
            lock (_lock)
            {
                return Words.OrderByDescending(w => w.Key.Length)
                    .OrderByDescending(w => w.Key).Take(5).Select(w => w.Key);
            }
        }
        ...
        public IEnumerable<string> GetSmallestFiveWords()
        {
            lock (_lock)
            {
                return Words.OrderBy(w => w.Key.Length).OrderBy(w => w.Key).Take(5).Select(w => w.Key);
            }
        }
